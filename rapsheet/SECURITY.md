...
**A problem with Rapsheet itself.** A wrong indicator that is causing false positives in a downstream tool, an entry that names something legitimate, or a flaw in the build or validation scripts.
...
Rapsheet is static JSON files, a JSON Schema and two Python scripts published through GitHub releases. There is no server, no database and no user data. Reports about the security of infrastructure Rapsheet does not run belong with whoever runs it.
