#!/usr/bin/env python3
"""Validate every RADAR entry against the schema and the house rules.
...
ID_RE = re.compile(r"^RADAR-\d{4}-\d{4}$")
...
def check_house_rules(path: Path, entry: dict) -> list[str]:
    ...
    if entry_id and ID_RE.match(entry_id) and first_seen:
        if entry_id[6:10] != first_seen[:4]:
            fails.append(
                f"{name}: field 'id' year {entry_id[6:10]} does not match "
