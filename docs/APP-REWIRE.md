...
Old entry shape on the left, RADAR on the right.

| Old field | New field | Notes |
| --- | --- | --- |
| `id` | `id` | Format changed: `ms-001-cloud-billing-autopilot` → `RADAR-2025-0001`. No old id survives; there is no migration table because no old entry survives either. |
...
| `name` (top level) | `feed` | Value is `"radar"`. |
