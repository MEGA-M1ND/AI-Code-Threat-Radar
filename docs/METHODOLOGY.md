...
An incident belongs in RADAR when it is a supply-chain threat to people using AI coding agents, and when it leaves behind at least one artifact a machine can match on. The six categories:
...
- **`malicious-package`** — a package that was malicious from its first published version. Nothing legitimate ever shipped under the name, so a guard should block the name outright rather than a version range. This is the distinction from `compromised-package`, and it changes what a consumer should do.
...
- **A CVE in agent tooling, catalogued as a CVE.** RADAR cites CVE, GHSA and OSV identifiers in `related`. Cataloguing them is [Georgia Tech SSLab's Vibe Security Radar](https://vibe-radar-ten.vercel.app/) lane. A `platform-vuln` entry exists here only when the incident matters as a supply-chain event and there is something to match on; it summarises in RADAR's own words and points at the CVE for the rest.
...
Every primary source is read in full before the entry is written. Nothing is quoted; entries are summarised in RADAR's own words.

## Indicator accuracy

This is the part that can hurt somebody. An indicator in this feed may end up in a runtime guard that blocks installs, so a wrong package name becomes a false positive in a tool RADAR does not control and cannot fix.
...
4. Outcome:
   ...
   - **Wrong entry** — the entry stays, with a summary saying what it got wrong and `status: disputed` retained. RADAR does not delete history, because a consumer that already ingested an indicator needs to see the retraction, not an absence.
